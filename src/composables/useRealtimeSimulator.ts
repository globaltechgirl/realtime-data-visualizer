...
import type { MetricPoint, ActivityEntry } from '../types';

const clamp = (value: number, min: number, max: number) => Math.max(min, Math.min(value, max));

const makeEvent = (data: MetricPoint): ActivityEntry => {
...
const randomWalk = (value: number, volatility: number, min: number, max: number) => {
